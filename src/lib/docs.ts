...
export function renderReadUsageText(): string {
  return [
    ...
    "- Helper aliases also exist for `resolve-billing-contact`, `find-invoice`, `find-transaction` and `match-transaction` under `read`.",
...
export function renderInvoiceEditWorkflowText(): string {
  return [
    ...
    "",
    "4. For content changes, prefer a safe recreate workflow instead of unsupported direct mutation:",
    "   sevdesk-agent invoice recreate --from <id> --patch-file payloads/invoice.patch.json --verify",
    "",
    "5. If needed, reset/cancel according to accounting rules before replacing the invoice in downstream processes.",
    "",
    "Tip:",
